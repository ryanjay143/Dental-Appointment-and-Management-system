                        ...
                        <div class="col-xl-6 col-lg-7">
                            <div class="card shadow mb-5 mt-5">
                                ...
                                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                                    <h6 class="m-0 font-weight-bold text-info">Edit Treatments</h6>
                                </div>

                                <!-- Card Body -->
                                <div class="card-body">
                                   <form action="{{ url('/treatments/'.$treatments->treatment_id ) }}" method="post" enctype="multipart/form-data">
                                        {!! csrf_field() !!}
                                        @method("PATCH")
                                            <div class="mb-3">
                                                <input type="text" class="form-control" id="name" name="name" placeholder="Treatment Name" value="{{ $treatments->name }}">
                                            </div>
                                            <div class="mb-3">
                                                <label for="formGroupExampleInput2" class="form-label">Image</label>
                                                <input type="file" class="form-control" id="image" name="image" value="{{ $treatments->image }}">
                                            </div>
                                            <div class="mb-3">
                                                <label for="exampleFormControlTextarea1" class="form-label">Description</label>
                                                <textarea class="form-control" id="description" name="description" rows="3">{{ $treatments->description }}</textarea>
                                            </div>
                                            <div class="input-group mb-3">
                                                <span class="input-group-text">&#8369;</span>
                                                <input type="text" class="form-control" aria-label="Amount (to the nearest dollar)" placeholder="Price" id="price" name="price" value="{{ $treatments->price}}">
                                                <span class="input-group-text">.00</span>
                                            </div>
                                            <div class="d-grid gap-2 d-md-block float-end">
                                                <a href="{{ route('dental.treatments') }}" class="btn btn-danger" type="button">Back</button></a>
                                                <button class="btn btn-info" type="submit" value="Update">save</button>
                                            </div>
                                    </form>
                                </div>
                            </div>
